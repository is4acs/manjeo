...
function validateMenu(menu: RestaurantMenu): string {
  ...
  for (const product of menu.products) {
    ...
    if (!product.archived && !categories.includes(product.group.trim())) return `Choisissez une catégorie pour « ${product.name} ».`;
    if (!Number.isInteger(product.price) || product.price < 1 || product.price > 100000) return `Le prix de « ${product.name} » doit être compris entre 0,01 € et 1 000 €.`;
    if (!validImage(product.image ?? "")) return `Utilisez une URL HTTPS ou importez une photo pour « ${product.name} ».`;
    for (const group of product.optionGroups) {
      if (!group.name.trim() || !group.choices.length) return `Nommez chaque groupe d’options de « ${product.name} » et ajoutez au moins un choix.`;
      if (!Number.isInteger(group.min) || !Number.isInteger(group.max) || group.min < 0 || group.max < 1 || group.min > group.max || group.max > group.choices.length) return `Vérifiez les limites de choix pour « ${group.name} » : minimum ≤ maximum ≤ nombre de choix.`;
      for (const choice of group.choices) if (!choice.name.trim() || !Number.isInteger(choice.price) || choice.price < 0 || choice.price > 100000) return `Chaque choix de « ${group.name} » doit avoir un nom et un supplément entre 0 € et 1 000 €.`;
    }
  }
  return "";
}
...
export default function MenuEditor({ restaurant, onRestaurantChange }: { restaurant: Restaurant; onRestaurantChange: (restaurant: Restaurant) => void }) {
  ...
  if (loading) return <div className="staff-loading" role="status"><RefreshCw size={24} className="staff-spinning" /><p>Chargement de votre carte…</p></div>;
  if (!draft) return <div className="staff-alert" role="alert"><span>{error || "La carte est indisponible."}</span><button type="button" onClick={() => void load(true)}>Réessayer</button></div>;
  const visible = draft.products.filter(product => showArchived || !product.archived);
  return <div className="menu-editor">
    <div className="staff-section-heading"><div><h2>Votre carte, à votre façon</h2><p>Produits, catégories, photos et personnalisations : publiez quand tout est prêt.</p></div></div>
    {error && <div className="staff-alert" role="alert"><span>{error}</span><button type="button" onClick={() => setError("")}>Fermer</button></div>}
    {notice && <p className="menu-saved" role="status"><CheckCircle2 size={17} />{notice}</p>}
    {conflict && <div className="menu-conflict" role="alert"><strong>La carte a changé depuis l’ouverture de votre brouillon.</strong><p>Vos modifications restent dans cet onglet. Copiez les éléments à conserver avant de charger la carte publiée ; aucun écrasement automatique n’est effectué.</p><button className="staff-button" type="button" onClick={() => setDiscardArmed(true)}>Charger la carte publiée</button></div>}
    {discardArmed && <div className="menu-conflict" role="alert"><strong>Remplacer le brouillon par la carte publiée ?</strong><p>Les modifications non publiées de cet onglet seront supprimées.</p><div className="menu-toolbar-actions"><button className="staff-button" type="button" onClick={() => setDiscardArmed(false)}>Garder mon brouillon</button><button className="staff-button menu-danger" type="button" onClick={() => void load()}>Abandonner et recharger</button></div></div>}
    <form onSubmit={publish}>
      <div className="menu-toolbar"><div><strong className={dirty ? "menu-dirty" : ""}>{dirty ? "Modifications non publiées" : "Votre carte est publiée"}</strong><p>{draft.products.filter(product => !product.archived).length} produits · version {draft.version}{dirty && " · brouillon conservé dans cet onglet"}</p></div><div className="menu-toolbar-actions"><button type="button" className="staff-button" disabled={saving || !!uploading} onClick={() => dirty ? setDiscardArmed(true) : void load()}><RefreshCw size={15} />Recharger</button><button type="submit" className="staff-button staff-button-primary" disabled={!dirty || saving || !!uploading || conflict}><Save size={16} />{saving ? "Publication…" : "Publier la carte"}</button></div></div>
      <fieldset disabled={saving || !!uploading} className="menu-editor">
        <section className="menu-panel"><h3>Les catégories</h3><p>L’ordre ci-dessous est celui de la vitrine. Renommer une catégorie déplace aussi ses produits.</p><div className="menu-categories">{draft.categories.map((category, index) => <div className="menu-category-row" key={index}><input aria-label={`Nom de la catégorie ${index + 1}`} value={category} maxLength={80} required onChange={event => renameCategory(index, event.target.value)} /><button type="button" className="menu-icon-button" aria-label={`Monter la catégorie ${category}`} disabled={index === 0} onClick={() => moveCategory(index, -1)}><ArrowUp size={16} /></button><button type="button" className="menu-icon-button" aria-label={`Descendre la catégorie ${category}`} disabled={index === draft.categories.length - 1} onClick={() => moveCategory(index, 1)}><ArrowDown size={16} /></button><button type="button" className="menu-icon-button" aria-label={`Supprimer la catégorie vide ${category}`} title="Une catégorie doit être vide pour être supprimée." disabled={draft.categories.length === 1 || draft.products.some(product => !product.archived && product.group === category)} onClick={() => setDraft({ ...draft, categories: draft.categories.filter((_, position) => index !== position) })}><Trash2 size={15} /></button></div>)}</div><div className="menu-new-category"><input aria-label="Nouvelle catégorie" value={categoryName} maxLength={80} placeholder="Ex. Desserts maison" onChange={event => setCategoryName(event.target.value)} /><button type="button" className="staff-button" disabled={!categoryName.trim() || draft.categories.length >= 30 || draft.categories.some(category => category.toLocaleLowerCase() === categoryName.trim().toLocaleLowerCase())} onClick={() => { setDraft({ ...draft, categories: [...draft.categories, categoryName.trim()] }); setCategoryName(""); }}><Plus size={15} />Ajouter une catégorie</button></div></section>
        <section><div className="menu-section-title"><div><h3>Les produits</h3><p>Les produits archivés restent dans l’historique des commandes.</p></div><button type="button" className="staff-button" disabled={draft.products.length >= 100 || !draft.categories.length} onClick={addProduct}><Plus size={16} />Ajouter un produit</button></div><label className="menu-check" style={{ marginTop: 16 }}><input type="checkbox" checked={showArchived} onChange={event => setShowArchived(event.target.checked)} />Afficher les produits archivés ({draft.products.filter(product => product.archived).length})</label><div className="menu-products">
          {visible.map(product => <article className="menu-product-editor" key={product.id}><div className="menu-product-heading">{product.image && validImage(product.image) ? <img src={product.image} alt="" loading="lazy" referrerPolicy="no-referrer" /> : <span className="menu-product-picture"><UtensilsCrossed size={25} /></span>}<div><h4>{product.name || "Produit sans nom"}</h4><p>{product.group} · {money(Math.max(0, product.price))}</p><small className={product.archived ? "menu-archived" : ""}>{product.archived ? "Archivé" : product.available ? "Disponible" : "Indisponible"}</small>{product.optionGroups.length > 0 && <small>{product.optionGroups.length} groupe{product.optionGroups.length > 1 ? "s" : ""} d’options</small>}</div><button type="button" className="staff-button" aria-expanded={expanded === product.id} onClick={() => setExpanded(expanded === product.id ? null : product.id)}><Pencil size={14} />{expanded === product.id ? "Réduire" : "Modifier"}</button></div>
          {expanded === product.id && <div className="menu-product-form"><div className="menu-form-grid"><label className="menu-field">Nom du produit<input required value={product.name} maxLength={120} onChange={event => updateProduct(product.id, { name: event.target.value })} /></label><PriceField label="Prix du produit (€)" value={product.price} minimum={1} onChange={price => updateProduct(product.id, { price })} /><label className="menu-field">Catégorie<select value={product.group} onChange={event => updateProduct(product.id, { group: event.target.value })}>{draft.categories.map(category => <option key={category} value={category}>{category}</option>)}</select></label><label className="menu-field">Allergènes<input value={product.allergens} maxLength={500} placeholder="Ex. gluten, lait, arachides" onChange={event => updateProduct(product.id, { allergens: event.target.value })} /></label><label className="menu-field menu-field-wide">Description<textarea value={product.description} maxLength={1000} onChange={event => updateProduct(product.id, { description: event.target.value })} /></label><label className="menu-field menu-field-wide">Photo : adresse HTTPS<input value={product.image ?? ""} maxLength={2000} placeholder="https://…" onChange={event => updateProduct(product.id, { image: event.target.value })} /><small>Une photo importée peut également être utilisée. Les photos déjà présentes restent disponibles.</small></label><label className="menu-field menu-field-wide"><span><ImagePlus size={14} /> Ou importer une photo</span><input type="file" accept="image/jpeg,image/png,image/webp" aria-label={`Importer une photo pour ${product.name}`} onChange={event => { void upload(product.id, event.target.files?.[0]); event.target.value = ""; }} /><small>{uploading === product.id ? "Import en cours…" : "JPEG, PNG ou WebP · 1 Mo maximum · publiez ensuite la carte."}</small></label></div><div className="menu-checks"><label className="menu-check"><input type="checkbox" checked={product.available} onChange={event => updateProduct(product.id, { available: event.target.checked })} />Disponible à la commande</label><label className="menu-check"><input type="checkbox" checked={!!product.popular} onChange={event => updateProduct(product.id, { popular: event.target.checked })} />Mettre en avant</label></div>
            <div className="menu-section-title"><h3>Personnalisation du produit</h3><button type="button" className="staff-button" disabled={product.optionGroups.length >= 8} onClick={() => updateProduct(product.id, { optionGroups: [...product.optionGroups, { id: newId(), name: "Nouvelle option", min: 0, max: 1, choices: [{ id: newId(), name: "Premier choix", price: 0 }] }] })}><Plus size={14} />Ajouter un groupe</button></div><p className="menu-note">Exemple : « Accompagnement », minimum 1, maximum 1, puis les choix « Riz » ou « Frites ». Minimum 0 rend le groupe facultatif.</p><div className="menu-option-groups">{product.optionGroups.map(group => <div className="menu-option-group" key={group.id}><div className="menu-option-group-title"><strong>{group.name || "Groupe d’options"}</strong><button type="button" className="menu-icon-button" aria-label={`Retirer le groupe ${group.name}`} onClick={() => updateProduct(product.id, { optionGroups: product.optionGroups.filter(item => item.id !== group.id) })}><Trash2 size={15} /></button></div><div className="menu-form-grid"><label className="menu-field menu-field-wide">Nom du groupe<input value={group.name} required maxLength={100} onChange={event => updateGroup(product, group.id, { name: event.target.value })} /></label><label className="menu-field">Minimum de choix<input type="number" min={0} max={group.choices.length} step={1} required value={group.min} onChange={event => updateGroup(product, group.id, { min: Number(event.target.value) })} /></label><label className="menu-field">Maximum de choix<input type="number" min={1} max={Math.max(1, group.choices.length)} step={1} required value={group.max} onChange={event => updateGroup(product, group.id, { max: Number(event.target.value) })} /></label></div><div className="menu-option-choices">{group.choices.map(choice => <div className="menu-option-choice" key={choice.id}><label className="menu-field">Choix<input value={choice.name} required maxLength={100} onChange={event => updateChoice(product, group, choice.id, { name: event.target.value })} /></label><PriceField label="Supplément (€)" value={choice.price} onChange={price => updateChoice(product, group, choice.id, { price })} /><button type="button" className="menu-icon-button" aria-label={`Retirer le choix ${choice.name}`} disabled={group.choices.length <= 1} onClick={() => { const choices = group.choices.filter(item => item.id !== choice.id); updateGroup(product, group.id, { choices, min: Math.min(group.min, choices.length), max: Math.min(group.max, choices.length) }); }}><Trash2 size={15} /></button></div>)}</div><button type="button" className="staff-button" disabled={group.choices.length >= 20} onClick={() => updateGroup(product, group.id, { choices: [...group.choices, { id: newId(), name: "Nouveau choix", price: 0 }] })}><Plus size={14} />Ajouter un choix</button></div>)}</div>
            <div className="menu-product-actions"><div className="menu-toolbar-actions"><button type="button" className="menu-icon-button" aria-label={`Monter le produit ${product.name}`} disabled={draft.products[0]?.id === product.id} onClick={() => moveProduct(product.id, -1)}><ArrowUp size={16} /></button><button type="button" className="menu-icon-button" aria-label={`Descendre le produit ${product.name}`} disabled={draft.products[draft.products.length - 1]?.id === product.id} onClick={() => moveProduct(product.id, 1)}><ArrowDown size={16} /></button></div><button type="button" className={`staff-button ${product.archived ? "" : "menu-danger"}`} onClick={() => { updateProduct(product.id, { archived: !product.archived, ...(product.archived && !draft.categories.includes(product.group) ? { group: draft.categories[0] } : {}) }); if (!product.archived) setShowArchived(true); }}>{product.archived ? <Undo2 size={15} /> : <Archive size={15} />}{product.archived ? "Restaurer ce produit" : "Archiver ce produit"}</button><small className="menu-note">Les changements prennent effet à la publication.</small></div>
          </div>}</article>)}
          {!visible.length && <p className="menu-empty-group">Aucun produit à afficher. Ajoutez votre premier produit ou affichez les archives.</p>}
        </div></section>
      </fieldset>
    </form>
    <section className="menu-panel"><h3>Votre restaurant et le retrait des commandes</h3><p>Ces informations sont enregistrées séparément de votre carte. Utilisez des coordonnées fictives pour cette démonstration.</p><form onSubmit={saveProfile}><fieldset disabled={profileBusy} className="menu-editor"><div className="menu-form-grid"><label className="menu-field">Nom du restaurant<input required value={profile.name} maxLength={120} onChange={event => setProfile({ ...profile, name: event.target.value })} /></label><label className="menu-field">Temps de préparation estimé (minutes)<input required type="number" min={5} max={180} step={1} value={profile.minutes} onChange={event => setProfile({ ...profile, minutes: Number(event.target.value) })} /></label><label className="menu-field menu-field-wide">Présentation<textarea value={profile.description} maxLength={500} onChange={event => setProfile({ ...profile, description: event.target.value })} /></label><label className="menu-field">Adresse de retrait<input required value={profile.pickupAddress} minLength={5} maxLength={250} onChange={event => setProfile({ ...profile, pickupAddress: event.target.value })} /></label><label className="menu-field">Commune de retrait<select value={profile.pickupCity} onChange={event => setProfile({ ...profile, pickupCity: event.target.value })}>{cities.map(city => <option key={city}>{city}</option>)}</select></label></div><div><button type="submit" className="staff-button" disabled={profileBusy}><Save size={15} />{profileBusy ? "Enregistrement…" : "Enregistrer les informations"}</button></div></fieldset></form></section>
  </div>;
}
