...
export default function MenuEditor({ restaurant, viewerId, onRestaurantChange }: { restaurant: Restaurant; viewerId: string; onRestaurantChange: (restaurant: Restaurant, fields: (keyof RestaurantProfile)[]) => void }) {
  ...
  async function load(restore = false) {
    ...
    try {
      const { menu } = await api<{ menu: RestaurantMenu }>(`/api/restaurants/${encodeURIComponent(restaurant.id)}/menu`, {accountId: viewerId});
  ...
  function addProduct() {
    ...
    const id = newId(); setDraft({ ...draft, products: [...draft.products, { id, name: t("Nouveau produit"), description: "", price: 100, group: draft.categories[0] || t("À la carte"), image: "", available: true, archived: false, allergens: "", version: 1, optionGroups: [] }] }); setExpanded(id);
  }
  async function publish(event: React.FormEvent) {
    ...
    try {
      const { menu } = await api<{ menu: RestaurantMenu }>(`/api/restaurants/${encodeURIComponent(restaurant.id)}/menu`, { method: "PATCH", accountId: viewerId, body: JSON.stringify(cleaned) });
    ...
    } catch (cause) { if (active.current) { setError(cause instanceof Error ? cause.message : "La publication a échoué."); if (cause instanceof ApiError && cause.status === 409 && cause.code !== "session_changed") setConflict(true); } }
  ...
  async function upload(productId: string, file: File | undefined) {
    ...
    try {
      ...
      if (!active.current) return;
      const data = await api<{ url: string }>(`/api/restaurants/${encodeURIComponent(restaurant.id)}/images`, { method: "POST", accountId: viewerId, body: JSON.stringify({ dataUrl }) });
  ...
  async function saveProfile(event: React.FormEvent) {
    ...
    try {
      const data = await api<{ restaurant: Restaurant }>(`/api/restaurants/${encodeURIComponent(restaurant.id)}`, { method: "PATCH", accountId: viewerId, body: JSON.stringify(changes) });
      if (active.current) {
