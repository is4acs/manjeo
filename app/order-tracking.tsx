...
import { clockLabel, countdown, useCountdown } from "./deadline";
...
function Countdown({deadline}: {deadline: string}) {
  ...
  return <p className="tracking-deadline"><Clock3 size={16}/><span>{seconds > 0
    ...
    : <>Le délai de dix minutes est écoulé : l’annulation automatique s’applique au prochain rafraîchissement.</>}</span></p>;
}
...
export default function OrderTracking({order, onCancel}: {order: Order; onCancel?: (order: Order) => void}) {
  ...
  return <section className="customer-tracking" aria-label={`Suivi ${order.id}`}>
    ...
    {order.eta && !["delivered", "cancelled"].includes(order.status) && <p className="tracking-deadline"><Clock3 size={16}/><span>Livraison estimée vers <strong>{clockLabel(order.eta)}</strong>{Date.parse(order.eta) < Date.now() ? " — le retard est signalé au restaurant et au livreur." : "."}</span></p>}
    ...
    {order.courierName && <p className="tracking-courier"><Bike size={18}/><span><strong>{order.courierName}</strong>{order.status === "picked_up" ? " apporte votre commande." : order.status === "delivered" ? " a assuré votre livraison test." : " a pris en charge votre course."}</span></p>}
