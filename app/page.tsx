  ...
  return <>
    ...
    <main className="page-shell">
      ...
      <div className={`main-layout ${view === "success" ? "single-layout" : ""}`}><div className="main-content">
        ...
        {view === "success" && currentOrder && <div className="success-page"><div className="success-icon"><CheckCheck size={43}/></div><span className="eyebrow">SUIVI DE VOTRE COMMANDE</span><h1>{currentOrder.status === "cancelled" ? "Commande annulée." : currentOrder.status === "delivered" ? "Bon appétit !" : "À table, bientôt."}</h1><p>Le restaurant et le livreur partagent ici chaque étape de votre commande.</p><div className="order-ticket"><div><span className="ticket-label">COMMANDE TEST</span><strong>{currentOrder.id}</strong></div><span className="ticket-status">{statusLabels[currentOrder.status]}</span><h2>{currentOrder.restaurant}</h2>{currentOrder.items.map((item, index) => <p key={index}>{item.quantity} × {item.name}{item.option && ` (${item.option})`}</p>)}<div className="ticket-total"><span>{currentOrder.count} article{currentOrder.count > 1 ? "s" : ""} · Livraison à {currentOrder.city}</span><strong>{money(currentOrder.total)}</strong></div><OrderTracking order={currentOrder} onCancel={requestCancel}/></div>{ordersError && <p className="orders-error" role="alert">{ordersError}</p>}<div className="success-info"><PackageCheck size={21}/><p>Testez la préparation depuis le compte restaurant, puis la prise en charge depuis le compte livreur.<br/>Votre suivi s’actualise ici. Aucun paiement ni livraison réelle.</p></div><Button className="primary-btn" onClick={() => setView("home")}>Découvrir d’autres adresses <ArrowRight size={17}/></Button><button className="text-link" onClick={openHistory}>Voir mes commandes test</button></div>}
      ...
      <footer className="site-footer"><span className="footer-brand">manjéo<span>✳</span></span><span>La Guyane a bon goût.</span><button onClick={openHistory}>Mes commandes test</button><span>Démo en ligne · aucune commande réelle</span></footer>
    </main>
