...
- 71 tests client réussis : traductions, paramètres, formulaires, requêtes de traduction, annulations réseau et changements de session entre onglets. TypeScript et build Vercel réussis. Vite signale un bundle principal supérieur à 500 ko minifié ; aucun échec de compilation.
...
Les derniers contrôles ont également validé la conservation des langues de messages historiques (`en`, `es`, `gcr`, `zh`) lors d’une modification des coordonnées, la reprise d’un seul message en erreur à la fois et les catégories/options de carte restaurateur à 320, 390 et 430 px en haïtien et portugais.
